...
class LIED:
    ...
    def __init__(self, E, Up, t0, L, WL, I0, TARGET, data_dir_path=r'.', E0f=None):
        self.E = E # eV
        self.Up = Up # eV
        self.t0 = t0 # sec
        self.L = L # m
    ...
    # Plotting
    def creat_dcs_fig(self, title=None, E0f=None, tfontsize=12, y_scale="log",\
                      xlabel=None, ylabel=None, fontsize=11, fontweight='bold'):
        # Labeling
        self.fig_dcs, self.ax_dcs = plt.subplots()
        if xlabel is None:
            xlabel='Scattering angle [deg]'
        if ylabel is None:
            ylabel='Differential Cross-Section [arb. unit]'
    ...
    def creat_mom_trans_fig(self, title=None, E0f=None, tfontsize=12, y_scale="log",\
                        xlabel=None, ylabel=None, fontsize=11, fontweight='bold'):
        self.fig_mt, self.ax_mt = plt.subplots()
        # Labeling
        if xlabel is None:
    ...
    def plot_dcs(self,ax=None, marker='o', linestyle='', markersize=7, line_width=2, capsize=4, color='blue', label='LIED', errorbar=True):
        ...
        if ax is None:
            ax = self.ax_dcs
        if errorbar:
            ax.errorbar(self.scatter_angle_deg_arr, self.dcs, self.error, linestyle=linestyle, lw=line_width, marker=marker, capsize=capsize, color=color, markersize=markersize, label=label)
        else:
            ax.plot(self.scatter_angle_deg_arr, self.dcs, linestyle=linestyle, lw=line_width, marker=marker, color=color, markersize=markersize, label=label)
    ...
    def plot_momentum_transfer(self, ax=None, marker='o', linestyle='', markersize=7, line_width=2, capsize=4, color='blue', label='LIED', errorbar=True):
        ...
        if ax is None:
            ax = self.ax_mt
        if errorbar:
            ax.errorbar(self.momentum_transfer, self.dcs, self.error, marker=marker, linestyle=linestyle, markersize=markersize, capsize=capsize, lw=line_width, color=color, label=label)
        else:
            ax.plot(self.momentum_transfer, self.dcs, marker=marker, linestyle=linestyle, markersize=markersize, lw=line_width, color=color, label=label)
    ...
    def run(self, delta=0.05, apply_jacobian=True, save=False, show=True, has_theory=False,\
        ...
        if show or save or plot:
            if ax is None:
                self.creat_dcs_fig(title=title, E0f=E0f, tfontsize=tfontsize, y_scale=y_scale,\
        ...
        if show_mt or save_mt or plot_mt:
            if ax_mt is None:
                self.creat_mom_trans_fig(title=title, E0f=E0f, tfontsize=tfontsize, y_scale=y_scale,\
